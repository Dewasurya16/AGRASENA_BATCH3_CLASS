...
export function SchedulesListB4() {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div className="space-y-1.5">
            <div className="flex items-center gap-2">
              ...
              <span className="rounded-full bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800 px-2.5 py-0.5 text-xs font-bold">
                Agrasena Batch 4
              </span>
            </div>
          ...
          <div className="flex items-center gap-2 self-start lg:self-center">
            <a
              href="#zoom-access"
              className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800/80 px-3.5 py-1.5 text-xs font-bold hover:bg-emerald-600 hover:text-white transition cursor-pointer"
            >
              <Video className="h-3.5 w-3.5" />
              <span>Akses Zoom Angkatan 1–6</span>
            </a>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pt-6 mt-4 border-t border-slate-100 dark:border-slate-800">
          ...
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="w-full rounded-full bg-slate-50 dark:bg-[#101520] border border-slate-200 dark:border-slate-800 pl-10 pr-4 py-2 text-xs text-[#18181B] dark:text-white placeholder-slate-400 focus:outline-hidden focus:border-emerald-500 transition"
            />
          </div>
      ...
      {/* 2. Akses Ruang Zoom Meeting Tiap Angkatan (Gambar 3 untuk Batch 4) */}
      <div id="zoom-access">
        <ZoomClassAccess batchNum={4} />
      </div>

      {/* 3. List of 35 Days Schedules */}
      <div className="space-y-3">
        {filteredSchedules.length === 0 ? (
          <div className="rounded-[20px] bg-white dark:bg-[#151c28] border border-dashed border-slate-300 dark:border-slate-800 p-12 text-center space-y-3">
            ...
            <h3 className="text-base font-bold text-[#18181B] dark:text-white">Belum Ada Sesi Rinci Jadwal Agrasena Batch 4</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 max-w-md mx-auto leading-relaxed">
              Jadwal perkuliahan 35 hari untuk angkatan ini sedang dipersiapkan dan akan dipublikasikan oleh Panitia Diklat / Widyaiswara Badiklat Kejaksaan RI. Sesi tatap muka dan link Zoom dapat diakses langsung pada ruang kelas online di atas.
            </p>
          </div>
        ) : (
          filteredSchedules.map((sched, idx) => {
            ...
            if (dayNum >= 6 && dayNum <= 15) {
              ...
              stageBadgeColor = "bg-teal-50 text-teal-700 dark:bg-teal-950/40 dark:text-teal-300 border-teal-200 dark:border-teal-800"
            } else if (dayNum >= 31) {
              stageBadge = "Tahap 4: Seminar"
              stageBadgeColor = "bg-amber-50 text-amber-700 dark:bg-amber-950/40 dark:text-amber-300 border-amber-200 dark:border-amber-800"
            }
            ...
            return (
              <motion.div
                ...
                className="rounded-[16px] bg-white dark:bg-[#151c28] border border-slate-200 dark:border-slate-800 hover:border-emerald-500/50 p-4 sm:p-5 transition-all hover:shadow-xs"
              >
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                  <div className="flex items-start gap-3.5">
                    ...
                    <div className="flex flex-col items-center justify-center h-12 w-12 rounded-xl bg-slate-100 dark:bg-[#101520] border border-slate-200 dark:border-slate-800 text-[#18181B] dark:text-white shrink-0">
                      ...
                      <span className="text-base font-black text-emerald-600 dark:text-emerald-400">{dayNum}</span>
                    </div>
                  ...
                  <div className="flex items-center gap-2 self-end sm:self-auto shrink-0 pt-2 sm:pt-0">
                    {sched.meeting_link ? (
                      <a
                        ...
                        className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 dark:bg-emerald-950/50 hover:bg-emerald-600 hover:text-white text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800 px-3.5 py-1.5 text-xs font-bold transition cursor-pointer"
                      >
                        <Video className="h-3.5 w-3.5" />
                        <span>Link Zoom</span>
